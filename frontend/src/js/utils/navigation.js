...
class NavigationSystem {
    ...
    async fetchSections() {
    try {
        // CHANGE: Use the navigation-specific endpoint to get only active sections
        const response = await api.get('/api/sections/navigation');
        this.sections = response.success ? response.data : [];
        
        console.log('🧭 Navigation sections loaded:', this.sections.length, 
            'Active sections:', this.sections.map(s => s.name));
    } catch (error) {
...
async refresh() {
    console.log('🔄 Refreshing navigation sections...');
    try {
        await this.fetchSections();
        
        // Re-render only if sidebar is already initialized
        if (this.initialized) {
            this.renderSidebar();
            this.bindEvents();
        }
  ...
  <rect x="4" y="4" width="16" height="16" rx="3" ry="3" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    renderSectionLinks() {
    // CHANGE: Filter out inactive sections as extra safety (backend should already do this)
    const activeSections = this.sections.filter(section => section.isActive !== false);
    
    console.log('🧭 Rendering section links:', activeSections.length, 'sections');

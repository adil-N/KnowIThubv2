...
async initializeFlashBanner() {
    console.log('🚀 Initializing flash banner with vertical slides');
    ...
    try {
        ...
        // Fetch articles with excludeViewed parameter
        const sectionIds = [];
        ...
        console.log('🏆 Unviewed articles found:', allArticles.length);
        ...
        // Declare perArticleDuration before it's used
        const perArticleDuration = 12; // 10s display + 2s transition

        // ✅ SIMPLIFIED: Create slides like original but with better click handling
        const slidesHtml = allArticles.map((article, index) => {
            const delay = 5 + (index * perArticleDuration); // 5s = 3s wait + 2s buffer
            const articleId = article._id || article.id || article.articleId;
            
            return `
                <div class="flash-article-slide" style="animation-delay: ${delay}s;">
        ...
        // Total duration
        const totalDuration = allArticles.length * perArticleDuration;
        scrollContainer.style.setProperty('--total-duration', `${totalDuration}s`);

        // ✅ Add click event listeners to ensure links work (simplified)
        const articleLinks = scrollContainer.querySelectorAll('.flash-article');
        articleLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                // Don't prevent default - let the hash navigation work normally
                const articleId = link.dataset.articleId;
                console.log('🔗 Banner link clicked:', articleId);
            });
        });

        console.log('🖼️ Animation setup:', {
            articlesCount: allArticles.length,
            animationDelay: `${perArticleDuration}s per article`,
        ...
        console.log('🌟 Banner initialization complete with working links');
...
// Helper method for navigation (optional - add if needed)
...
// KEEP ONLY ONE updateBannerPosition method - remove the duplicate
